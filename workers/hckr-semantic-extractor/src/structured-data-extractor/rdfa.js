...
function parseElement(
  ...
  if (element.nodeType === 1) {
    ...
    if (element.getAttribute("typeof") === "rdfa:Pattern") {
      ...
      subject =
        evaluateSafeCURIEorCURIEorIRI(
          element.getAttribute("resource") || element.getAttribute("about"),
          prefixMappings,
          defaultPrefixMapping,
          baseIRI,
        ) || (element.getAttribute("typeof") ? dataModel.blankNode() : subject);
          ...
          .forEach(type => {
            if (element.getAttribute("property")) {
              element
                ...
                .split(/\s+/)
                .forEach(property => {
                  let predicate = evaluateTERMorCURIEorAbsIRI(
                    property,
                    vocabulary,
                    prefixMappings,
                    defaultPrefixMapping,
          ...
          .forEach(property => {
            ...
            if (property === "rdfa:copy") {
              if (element.getAttribute("href")) {
                ...
                parseElement(
                  document.querySelector(element.getAttribute("src")),
                  prefixMappings,
                  defaultPrefixMapping,
                  vocabulary,
                  subject,
        ...
        relation
          ...
          .forEach(rel => {
            if (element.getAttribute("src")) {
              ...
              invokeTarget(
                subject,
                evaluateTERMorCURIEorAbsIRI(relation, vocabulary, prefixMappings, defaultPrefixMapping),
                object,
              );
              relation = null;
        ...
        revrelation
          ...
          .forEach(rev => {
            if (element.getAttribute("src")) {
              ...
              invokeTarget(
                object,
                evaluateTERMorCURIEorAbsIRI(rev, vocabulary, prefixMappings, defaultPrefixMapping),
                subject,
              );
              revrelation = null;
            } else if (element.getAttribute("href")) {
...
function evaluateTERMorCURIEorAbsIRI(toCoAI, vocabulary, prefixMappings, defaultPrefixMapping) {
  ...
  return (
    evaluateTERM(toCoAI, vocabulary) ||
    evaluateCURIE(toCoAI, prefixMappings, defaultPrefixMapping) ||
    evaluateIRI(toCoAI)
  );
}
